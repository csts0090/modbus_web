...
<html lang="zh-TW">
  ...
  <body>
	...
	if ( $pid == -1 ) {       
		...
		$status = null;
	  	$query4 = "UPDATE Schedule SET pid=$pid where Schedule_id=$schedule_id";
	  	$result = mysqli_query($dbc, $query4) or die('Error querying database.');
		pcntl_waitpid($pid, $status);
	} else  {
		   // We are the child
		   // Do something with the inherited connection here
		   // It will get closed upon exit
		   //pcntl_exec('sudo /usr/bin/serial_read3', array('/var/www/demo/temp_file.txt')); 
		   exec('sudo /usr/bin/serial_read3 /var/www/demo/temp_file.txt'); 
		   //system("id");
		   //pclose(popen('/usr/bin/serial_read3 /var/www/demo/temp_file.txt &','r')); 
	  ...
	  /*	$query4 = "SELECT * FROM Schedule_item";
		...
		fclose($fp);
	  	mysqli_close($dbc);
		header("Location: http://140.127.196.105:3987/demo/res.php");
		//exit;
	?>
